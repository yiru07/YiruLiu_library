...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="author_info in authors" :key="author_info.id">
          {{ author_info.name }} ({{ author_info.birthYear }})
        </li>
      </ul>
      ...
      <!-- TODO: CODE TO RENDER LIST OF AUTHORS HERE -->
      <ul>
        <li v-for="author_info in modernAuthors" :key="author_info.id">
          {{ author_info.name }} ({{ author_info.birthYear }})
        </li>
      </ul>
      ...
      <ul>
        ...
        <li v-for="famouswork in allFamousWorks" :key="famouswork">
          {{ famouswork }}
        </li>
      </ul>
      ...
      <ul>
        <li v-for="famouswork in allAustenWorks" :key="famouswork">
          {{ famouswork }}
        </li>
      </ul>
    </section>
...
// Activity 9: Get Austen's works
const allAustenWorks = computed(() => {
  return authors
    .filter((author) => author.name.includes('Austen'))
    .flatMap((author) => author.famousWorks.map((work) => work.title))
})

// Activity 9a: Get bookstores' name
const bookstoreName = computed(() => {
  return bookstores.name;
})

// Activity 9b: Get total bookstores' number
const totalBookStoreNum = computed(() => {
